...
// Añadir al DOM
    ...
    var busqueda_pedidos_otros_canales_empresarios_facturar = function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (var i = 0; i < casos.length; i++) {
                    lineas += "<tr>" +
                        "<td><input type='checkbox' class='checkbox-enviar' name='pk' value='" + casos[i].pk + "'></td>" +
                        "<td>" + casos[i].num_pedido + "</td>" +
                        "<td>" + casos[i].cliente + "</td>" +
                        "<td>" +
                        "<input type='date' class='form-control' name='fechafacturar' value='" + fechaHoy + "'> " +
                        "</td>" +
                        "<td>" +
                        "<input type='hidden' name='items[" + i + "][pk]' value='" + casos[i].pk + "'>" +
                        "<input type='hidden' name='max_facturar' value='"+ casos[i].cantidadpendientefacturar +"'>"+
                        "<input type='number' class='form-control' min='0' max='" + casos[i].cantidadpendientefacturar +
                        "' name='cantidad' value='" + casos[i].cantidadpendientefacturar + "'>" +
                        "</td>" +
                        "<td class='cantidadpedido'>" + casos[i].cantidadpedido + "</td>" +
                        "<td class='cantidadrecibida'>" + casos[i].cantidadrecibida + "</td>" +
                        "<td class='cantidadfactura'>" + casos[i].cantidadfactura + "</td>" +
                        "<td>" + casos[i].fecha + "</td>" +
                        "<td>" + casos[i].u_plu + "</td>" +
                        "<td>" + casos[i].referencia + "</td>" +
                        "<td>" + casos[i].nombre + "</td>" +
        ...
        $("#select_all_factura").off("change").on("change", function () {
            $(".checkbox-enviar").prop("checked", this.checked);
        });
    }
    ...
    var enviar_referencia_facturar = function () {
        const csrfToken = document.querySelector('input[name="csrfmiddlewaretoken"]').value;
        const seleccionados = [];
        $(".checkbox-enviar:checked").each(function () {
            const fila = $(this).closest("tr");
            const pk = fila.find("input[name='pk']").val();
            const cantidad = parseInt(fila.find("input[name='cantidad']").val(), 10);
            const max_facturar = parseInt(fila.find("input[name='max_facturar']").val(), 10);
            const fecha_facturar = fila.find("input[name='fechafacturar']").val();

            if (!cantidad || cantidad <= 0) {
                alert("Debe ingresar una cantidad diferente a Cero por favor válida cada uno de los registros");
                throw new Error("Cantidad inválida");
            }

            if (cantidad > max_facturar) {
                alert("La cantidad ingresada excede la cantidad recibida. Por favor, verifique que sea menor o igual a la disponible para facturación " + max_facturar);
                throw new Error("Error en cantidad");
            }

            seleccionados.push({
                pk: pk,
                cantidad: cantidad,
                max_facturar:max_facturar,
        ...
        $.ajax({
            url: "/configuracion/facturacion/otros_canales/",
            type: "POST",
            data: JSON.stringify({seleccionados: seleccionados}),
            dataType: "json",
            contentType: "application/json",
            headers: {"X-CSRFToken": csrfToken},
            success: function (response) {
                alert("La cantidad a facturar ha sido registrada correctamente.");
                busqueda_pedidos_otros_canales_empresarios_facturar();
            },
            error: function (xhr) {
                let response = JSON.parse(xhr.responseText);
                if (response.error){
                    alert("Error de operación: " + response.error[0].status);
                }
                else {
                    alert("Error de operación: " + xhr.responseText);
                }
            }
        });
    ...
    $("#enviar_referencia_facturar").click(
        function (e){
            enviar_referencia_facturar()
        }
    )
            ...
            success: function (data) {
                ...
                for (let i = 0; i < casos.length; i++) {
                    ...
                    lineas += "<tr>" +
                        ...
                        // FECHA PARA CAPTURAR LA RECEPCIÓN
                        "<td>" +
                        "<input type='date' class='form-control' name='fecharecibir' value='" + fechaHoy + "'> " +
                        "</td>" +

                        // Input de cantidad obligatoria
                        "<td>" +
                        "<input type='hidden' name='max_recibir' value='" + casos[i].cantidad +"'> " +
                        ...
                        // CANTFAC (cantidad Facturada)
                        "<td>" + casos[i].cantidadfacturada + "</td>" +
                ...
                $("#enviar_seleccionados").off("click").on("click", function () {
                    ...
                    $(".check-envio:checked").each(function () {
                        ...
                        const max_recibir = parseInt(fila.find("input[name='max_recibir']").val(), 10);
                        const fecha_recibir = fila.find("input[name='fecharecibir']").val();

                        if (!cantidad || cantidad <= 0) {
                            alert("Debe ingresar una cantidad válida para todos los pedidos seleccionados.");
                            throw new Error("Cantidad inválida");
                        }
                        if (cantidad > max_recibir) {
                            alert("Error La cantidad ingresada excede la cantidad pendiente. Debe ser menor o igual a la cantidad pendiente. " + max_recibir);
                            throw new Error("Cantidad inválida");
                        }
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            alert("Notificación generada\n" +
                                "Recepción registrada. Correo enviado al proveedor.")
                            busqueda_pedidos_otros_canales_empresarios_recibo();
                        },
                        error: function (xhr) {
                            let response = JSON.parse(xhr.responseText);
                            if (response.error){
                                alert("Error en el envío de notificación\n" +
                                    "No fue posible completar el envío del correo. Detalle del error: " + response.error[0].status);
                            }
                            else {
                                alert("Error en el envío de notificación\n" +
                                    "No fue posible completar el envío del correo. Detalle del error: " + xhr.responseText);
                            }
                        }
                    });
                });
...
//BUSCAR SUBEXPEDIENTE PARA EDITAR
    ...
    //BUSCAR HISTORIAL RECEPCION
    var busqueda_historial_recepcion = function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (var i = 0; i < casos.length; i++) {
                    ...
                    const fecha = new Date(casos[i].fecha);
                    ...
                    lineas += "<tr>" +
                        ...
                        "<td> Pedido #" + casos[i].num_pedido + "</td>" +
                        "<td>" + casos[i].cliente + "</td>" +
                        "<td>" + casos[i].cantidadpedido + "</td>" +
                        "<td>" + casos[i].cantidadrecibida + "</td>" +
                        "<td>" + casos[i].cantidadrecibidaant + "</td>" +
                        "<td>" + casos[i].cantidadtotales + "</td>" +
                        "<td>" + casos[i].cantidadfacturadas + "</td>" +
                        "<td>" + casos[i].u_plu + "</td>" +
                        "<td>" + casos[i].referencia + "</td>" +
                        "<td>" + casos[i].nombre + "</td>" +
                        "<td>" + casos[i].codigo + "</td>" +
                        "<td>" + casos[i].empresa + "</td>" +
                        "<td>" + casos[i].observaciones + "</td>" +
                        "<td> <a title='Eliminar' class='btn btn-info btn-sm' href='/configuracion/historial_recepcion/borrar/" + casos[i].identificador +
    ...
    $("#busqueda_historial_recepcion").click(
        function (e) {
            busqueda_historial_recepcion()
        }
    )
            ...
            success: function (data) {
                ...
                for (let i = 0; i < casos.length; i++) {
                    lineas += "<tr>" +
                        ...
                        "<td>" + casos[i].referencia + "</td>" +
                        "<td>" + casos[i].nombre + "</td>" +
                        "<td><a href='/configuracion/consulta_historial_recepcion/detalle/" + casos[i].pk + "'  class='btn btn-block btn-info'>Ver</a></td>" +
                        "<td>" + casos[i].codigo + "</td>" +
                        "<td>" + casos[i].empresa + "</td>" +
    ...
    $("#busqueda_pedidos_otros_canales_consulta_recepcion").click(function (e) {
        busqueda_pedidos_otros_canales_consulta_recepcion();
    });

    // Imprimir Consulta Detalle Recepcion
    var consulta_recepcion_excel = function () {
    ...
    $("#consulta_recepcion_excel").click(
        function (e) {
            consulta_recepcion_excel()
        }
    )

    //BUSCAR HISTORIAL FACTURACION

    var busqueda_historial_facturacion = function () {
            ...
            success: function (data) {
                ...
                for (var i = 0; i < casos.length; i++) {
                    ...
                    lineas += "<tr>" +
                        ...
                        "<td>" + casos[i].cantidadrecibida + "</td>" +
                        "<td>" + casos[i].cantidadfacturada + "</td>" +
                        "<td>" + casos[i].cantidadfacturadaant + "</td>" +
                        "<td>" + casos[i].cantidadfacturadatotal + "</td>" +
                        "<td>" + casos[i].u_plu + "</td>" +
                        "<td>" + casos[i].referencia + "</td>" +
                        "<td>" + casos[i].nombre + "</td>" +
                        "<td>" + casos[i].codigo + "</td>" +
                        "<td>" + casos[i].empresa + "</td>" +
                        "<td> <a title='Eliminar' class='btn btn-info btn-sm' href='/configuracion/historial_facturacion/borrar/" + casos[i].identificador +
                        "'><i class='glyphicon glyphicon-trash'></i></a> </td>" +
                        "</tr>";
                }
                table_body.append(lineas)
    ...
    $("#busqueda_historial_facturacion").click(
        function (e) {
            ...
            success: function (data) {
                ...
                for (var i = 0; i < casos.length; i++) {
                    lineas += "<tr>" +
                        ...
                        "<td class='cantidadfactura'>" + casos[i].cantidadfactura + "</td>" +
                        "<td>" + casos[i].fecha + "</td>" +
                        "<td><a href='/configuracion/consulta_historial_facturacion/detalle/" + casos[i].pk + "'  class='btn btn-block btn-info'>Ver</a></td>" +
                        "<td>" + casos[i].u_plu + "</td>" +
                        "<td>" + casos[i].referencia + "</td>" +
                        "<td>" + casos[i].nombre + "</td>" +
                        "<td>" + casos[i].codigo + "</td>" +
    ...
    $("#busqueda_pedidos_otros_canales_consulta_facturacion").click(function (e) {
        busqueda_pedidos_otros_canales_consulta_facturacion();
    });

    // Imprimir Consulta Detalle Facturacion
    var consulta_facturacion_excel = function () {
    ...
    $("#consulta_facturacion_excel").click(
        function (e) {
            consulta_facturacion_excel()
        }
    )


    // CALENDARIO AGENDA DE TRABAJ
...
function obtener_paises(selectObject) {
    ...
    $.ajax({
        ...
        success: function (data) {
            // Cargar en Select
            paises = data.datos;
            var pais_valores = $(paises).filter(function (i, n) {
                return n.continentes === value
            });
...
function obtener_departamentos(selectObject) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var departamento_valores = $(departamentos).filter(function (i, n) {
                return n.paises === value
            });
...
function obtener_municipios(selectObject) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var municipio_valores = $(municipios).filter(function (i, n) {
                return n.departamentos === value
            });
...
function obtener_funcionarios(selectObject) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var funcionarios_valores = $(funcionario_destino).filter(function (i, n) {
                return n.dependencias === value
            });
...
function obtener_subseries(selectObject) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var subseries_valores = $(subseries).filter(function (i, n) {
                return n.series === value
            });
...
function obtener_empresa_tipo(selectObject) {
    ...
    $.ajax({
        ...
        success: function (data) {
            // Cargar en Select
            trd = data.datos;
            var trd_valores = $(trd).filter(function (i, n) {
                return n.tipo_empresa === value
            });
...
function obtener_subexpediente_ex(selectObject) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var subexpediente_valores = $(subexpediente).filter(function (i, n) {
                return n.expediente === value || n.expediente === 'NS'
            });
